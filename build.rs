...
fn build_config_file(config: &Config) -> std::io::Result<()> {
    ...
    match config.distribution.as_str() {
        "exp" => {
            writeln!(file, "use crate::types::ExpDistribution;")?;
            writeln!(file, "pub type Dist = ExpDistribution<Precision>;")?;
        }
        "weibull" => {
            writeln!(file, "use crate::types::WeibullDist;")?;
            writeln!(file, "pub type Dist = WeibullDist<Precision>;")?;
        }
        _ => panic!("Distribution {} not supported.", config.precision),
    }
...
fn build_job_file(config: &Config) -> std::io::Result<()> {
    ...
    content = content.replace("JOBNAME", &config.job_name);
    content = content.replace("N_THREADS", format!("{}", &config.n_threads).as_str());

    for p in config.params.iter() {
